...
public class TradeChestProtectionListener implements Listener {

    private final Main plugin;

    public TradeChestProtectionListener(Main plugin) {
        this.plugin = plugin;
    }
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        ...
        if (block.getType() == Material.CHEST && plugin.getTradeChestManager().isShop(block.getLocation())) {
            ...
            plugin.getTradeChestManager().removeShop(block.getLocation());
            player.sendMessage(plugin.getTradeChestConfigManager().msgShopRemoved());
            return;
        }

        // Player breaking a sign attached to a shop
        for (TradeChest shop : plugin.getTradeChestManager().getShops().values()) {
        ...
        if (isProtectedBlock(block.getLocation(), player)) {
            event.setCancelled(true);
            player.sendMessage(plugin.getTradeChestConfigManager().msgProtectedArea());
        }
    }
    ...
    public void onBlockPlace(BlockPlaceEvent event) {
        if (isProtectedBlock(event.getBlock().getLocation(), event.getPlayer())) {
            event.setCancelled(true);
            event.getPlayer().sendMessage(plugin.getTradeChestConfigManager().msgProtectedArea());
        }
    }
    ...
    private boolean isProtectedBlock(Location loc, Player player) {
        ...
        int radius = plugin.getTradeChestConfigManager().getProtectionRadius();
        if (!plugin.getTradeChestManager().isNearShop(loc, radius)) return false;

        for (Location shopLoc : plugin.getTradeChestManager().getShops().keySet()) {
