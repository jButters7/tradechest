...
public class ShopManager {
    ...
    public boolean createShop(Player player, Location chestLocation,
        ...
        if (isShop(chestLocation)) {
            player.sendMessage(plugin.getConfigManager().msgShopAlreadyExists());
            return false;
        }
        ...
        if (!player.hasPermission("tradechest.bypass.limit") && hasReachedShopLimit(player.getUniqueId())) {
            ...
            player.sendMessage(plugin.getConfigManager().msgShopLimitReached(limit));
            return false;
        }
        Shop shop = new Shop(player, chestLocation, sellingItem, sellingAmount, priceItem, priceAmount);
        shop.setNotificationsEnabled(plugin.getConfigManager().isSaleNotificationsDefault());
        shops.put(chestLocation, shop);
        shop.createSign();
        shop.createHologram(plugin.getConfigManager().getHologramHeight());
        saveShop(shop);
        return true;
    }

    public boolean removeShop(Location chestLocation) {
        Shop shop = shops.get(chestLocation);
    ...
    public boolean processPurchase(Player player, Location shopLoc) {
        Shop shop = shops.get(shopLoc);
        if (shop == null) {
            player.sendMessage(plugin.getConfigManager().msgShopDoesntExist());
            return false;
        }
        ...
        if (!shop.hasEnoughStock(chestInv, 1)) {
            player.sendMessage(plugin.getConfigManager().msgNotEnoughStock());
            return false;
        }

        int totalPrice = shop.getPriceAmount();
        if (!shop.playerCanPay(player, 1)) {
            player.sendMessage(plugin.getConfigManager().msgNotEnoughPayment(
                    totalPrice, Shop.formatItemName(shop.getPriceItem().getType())));
            return false;
        }
        ...
        player.sendMessage(plugin.getConfigManager().msgPurchaseSuccess(
                totalSell, Shop.formatItemName(shop.getSellingItem().getType()),
                totalPrice, Shop.formatItemName(shop.getPriceItem().getType())));
        ...
        if (shop.isNotificationsEnabled()) {
            ...
            if (owner != null && owner.isOnline()) {
                owner.sendMessage(plugin.getConfigManager().msgSaleNotification(
                        player.getName(), totalSell,
                        Shop.formatItemName(shop.getSellingItem().getType()),
                        totalPrice, Shop.formatItemName(shop.getPriceItem().getType())));
            }
        }

        shop.incrementTotalSales();
        shop.updateHologram(plugin.getConfigManager().getHologramHeight());
    ...
    private void loadShops() {
        ...
        for (String key : section.getKeys(false)) {
            ...
            try {
                ...
                Shop shop = new Shop(ownerUUID, ownerName, loc,
    ...
    private synchronized void saveShop(Shop shop) {
    ...
    private void saveShopAsync(Shop shop) throws IOException {
        String key = locationKey(shop.getChestLocation());
        ConfigurationSection s = shopsConfig.createSection("shops." + key);
        writeShopToSection(s, shop);
    ...
    private void writeShopToSection(ConfigurationSection s, Shop shop) {
        ...
        for (var world : Bukkit.getWorlds()) {
            for (Entity e : world.getEntities()) {
                if (e instanceof ArmorStand as) {
                    if (as.getPersistentDataContainer().has(Shop.HOLOGRAM_KEY,
    ...
    public void validateAllShops() {
        ...
        for (Map.Entry<Location, Shop> entry : shops.entrySet()) {
            ...
            if (loc.getBlock().getType() != Material.CHEST) {
                ...
                entry.getValue().createHologram(plugin.getConfigManager().getHologramHeight());
            }
        }
        for (Location loc : invalid) {
            Shop s = shops.remove(loc);
    ...
    public Map<Location, Shop> getShops() { return shops; }

    public boolean isOwner(Player player, Location loc) {
        Shop s = shops.get(loc);
    ...
    public int getPlayerShopLimit(UUID uuid) {
        return customShopLimits.getOrDefault(uuid, plugin.getConfigManager().getMaxShopsPerPlayer());
    }
    ...
    public void updateAllHolograms() {
        double h = plugin.getConfigManager().getHologramHeight();
